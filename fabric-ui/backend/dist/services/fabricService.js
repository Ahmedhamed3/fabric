...
export class FabricService {
    ...
    runPeerCmd(envScript, cmd, timeout = 15000) {
    ...
    async getContainers() {
        ...
        const details = await Promise.all(target.map(async (container) => {
            const inspect = await runCommand('docker', ['inspect', container.Names, '--format', '{{range $k,$v := .NetworkSettings.Networks}}{{$k}} {{$v.IPAddress}}{{end}}'], 9000);
    ...
    async getChaincode() {
        const errors = [];
        const committed = await this.runPeerCmd(`${config.composeDir}/env_org1.sh`, `peer lifecycle chaincode querycommitted -C ${config.channel} -n ${config.ccName}`, 14000);
        ...
        if (committed.exitCode !== 0)
            errors.push('querycommitted failed');
        if (installed.exitCode !== 0)
            errors.push('queryinstalled failed');
    ...
    async getOverview() {
        ...
        if (services.length < requiredServices.length)
            errors.push('One or more required services are missing.');
        if (!chaincode.ccaaSReachable)
            errors.push('CCaaS reachability check failed.');
